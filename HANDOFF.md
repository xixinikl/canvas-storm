...
```text
origin -> https://github.com/2082743849-beep/canvas-storm.git
```

用户要求上传到“xixinicl 那个仓库”，但本地没有该远端；`gh` 查询结果：

- `xixinicl` 不是可识别的 GitHub user/org。
- `https://github.com/xixinicl/canvas-storm.git` 不存在或无权限。
- 当前登录 GitHub 账号为 `inernoro`。
- `inernoro` 下暂未找到 `xixinicl` 或 `canvas-storm` 仓库。

上传前需要确认准确目标仓库 URL，或确认是否要新建仓库。
