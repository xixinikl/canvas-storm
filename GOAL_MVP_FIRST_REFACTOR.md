...
| PR1 | 预览/分支卫生收口 | 未开始 | 25% | 现有预览 PR 状态明确，本地超前提交处理完毕 |
| PR2 | MVP-first 数据模型 | 未开始 | 38% | 新旧项目均可读取，新项目包含 MVP、directions、exploration nodes |
| PR3 | AI/fallback 输出契约 | 未开始 | 50% | AI 和本地降级都返回 MVP-first 结构，测试覆盖解析和容错 |
| PR4 | 首屏信息架构 | 未开始 | 62% | 首屏清楚展示 MVP 主线和方向选择，只有一个主动作 |
| PR5 | 方向画布与交互语义 | 未开始 | 75% | 选择方向、补充当前层、进入下一层三种行为稳定且不混淆 |
| PR6 | 持久化、导出和测试补强 | 未开始 | 88% | 用户项目保存/恢复、备选库和导出适配新结构，`npm test` 通过 |
| PR7 | 浏览器验收和合并准备 | 未开始 | 100% | 桌面/移动端主路径走通，PR 描述、测试证据、风险边界齐全 |
