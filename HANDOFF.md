...
- 中心想法有时不够清晰。
- 子节点在某些项目/窗口尺寸下会重叠。
- 产品逻辑仍然不如“先生成最小 MVP，再按方向发散”清楚。
- 当前画布更像被多次修补后的原型，不适合继续堆 UI 小修。

## 建议下一步重构

优先做信息架构重构，不建议继续只调 CSS。
...
1. 提交并推送 PR0：当前状态、规划、交接和 Goal 文件已验证。
2. 继续 PR1：收口预览 PR 和分支卫生。
3. 按 `PR_PLAN.md` 推进下一轮产品重构。
4. 优先重构产品模型：
   - 先生成 MVP 主线。
   - 再生成发散方向。
   - 方向内按层级生成节点。
5. 修复当前画布节点重叠问题，或直接用新的层级布局替换现有自由坐标布局。
6. 将 UI 文案统一为：
   - MVP 主线
   - 发散方向
   - 当前节点
